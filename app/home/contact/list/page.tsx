...
const List=()=>{
    ...
    async function addtotrash(id:any) {
        const path=`addtrash/${id}`
        const option={
            method:"PUT"
        }
  ...
  async function getcontacts() {
    ...
    setData(data);
    localStorage.setItem("lengthcontact",JSON.stringify(data.length))
        ...
        <tbody>
            { data?.map((item:any,index:number)=>(
                ...
                <td className="px-6 flex justify-start items-center py-4">
                   <Link href={`/home/contact/edit/${item._id}`}>
                   <Image className="w-4 h-4" src={edit} alt="edit"/>
                   </Link>
                   <div  onClick={()=>addtotrash(item?._id)} className="ml-4 cursor-pointer">
